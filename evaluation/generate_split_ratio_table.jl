...
results = DataFrame(skus=Int[], ratio=Float64[], heuristic=String[], test_split_ratio=Any[], train_split_ratio=Any[])

println("Average split ratio (in %) depending on the number of SKUs with train/test comparison:")
println("="^60)
        ...
        for ratio in unique_ratios
            for heur in heuristics
                ...
                if actual_test_ratio !== nothing
                    push!(results, (skus=sku, ratio=ratio, heuristic=heur, test_split_ratio=actual_test_ratio, train_split_ratio=actual_train_ratio))
                end
            end
        end
    end
...
println("      \\smaller")
println("      \\item \\textit{Notes.} Split ratios as percentages are shown for each algorithm (lower is better). Test performance is shown in the first row, training performance in the second row (smaller font). Only algorithms with 100\\% success rates are shown. We used an octa-core AMD 5800X3D CPU with 64 GB RAM.")
