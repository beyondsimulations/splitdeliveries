function CHISQUAREHEUR(trans::SparseMatrixCSC{Bool,Int64},
    ...
    if CHECKCAPACITY(capacity,sku_weight)
        ...
        ## Allocate the SKUs with coapperances
        n_allocated = Ref(0)
        ...
        ## Recompute warehouse weights after replication changed capacity landscape
        if n_allocated[] > 0
            weight = WHWEIGHT(round.(Int64, cap_left), sum_nor, sku_weight)
        end
        while n_allocated[] < I
        ...
        i,k  = SELECTIK(sum_dep,sum_nor,weight,cap_left,state_dep,allocated,sku_weight,nor_order,nor_pos)
        ALLOCATEONE!(X,dep,Q,sum_dep,sum_nor,state_dep,state_nor,cap_left,allocated,sku_weight,i,k,n_allocated)
        ...
        ## the warehouse k.
        ADDDEPENDENT!(X,Q,cap_left,k,dep,sum_dep,sum_nor,state_dep,state_nor,allocated,sku_weight,n_allocated)
        FILLLAST!(X,cap_left,allocated,sku_weight,n_allocated)
        end
        if sum(cap_left) > 0
        ...
        if max_ls > 0
            if all(y->y==sku_weight[1],sku_weight)
                ...
                ls = LOCALSEARCHCHI!(trans,X,Q,capacity,log_results,ls,max_ls,ls_neighborhood)
            else
                @warn "Local search requires uniform SKU weights — skipping. Results reflect CHI only."
            end
        end
        # Check whether all SKUs are allocated
        if any(y->y < 1,sum(X,dims=2))
            error("CHI: Not all SKUs are allocated!")
        end
    end
