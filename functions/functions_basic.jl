...
function SWAPREPAIR!(X::Matrix{Bool}, cap_left::Vector{<:Real},
                     sku_weight::Vector{<:Real}, heavy_sku::Int64)
    ...
    error("SWAPREPAIR!: Cannot fit SKU $heavy_sku (weight $w_heavy) in any warehouse via swap.")
end
...
function RANDOMALLOCONCE(
    ...
    for j in order
        ...
        isempty(valid) && error("No warehouse with enough capacity for SKU $j (weight $(sku_weight[j]), max=$(maximum(cap_left)))")
        k = rand(valid)
        X[j, k] = true
...
function RANDOMALLOCMULTI(
    ...
    for d in axes(capacity,1)
        available = findall(i -> !X[i,d] && sku_weight[i] <= capacity[d] - cap_used[d], 1:size(trans,2))
