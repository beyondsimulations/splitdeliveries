...
function ADDDEPENDENT!(X::Array{Bool,2},
    ...
    while add == 1 && cap_left[k] > 0 && sum(X) < size(X,1)
        add = 0
        pot_test = Vector{Float64}(undef,size(X,1)) .= 0
        FINDDEP!(X::Array{Bool,2},
                            ...
                            pot_dep::Matrix{Float64},
                            pot_nor::Matrix{Float64},
                            pot_test::Vector{Float64})
        best = findmax(pot_test)[1]
        i    = findmax(pot_test)[2]
        if best > 0
            if pot_dep[i] >= findmax(pot_nor[:,k])[1]
...
function FINDDEP!(X::Array{Bool,2},
    ...
    @inbounds @simd for j in 1:size(X,1)
        if allocated[j] == 0
            ...
            pot_nor[j,k] += nor[j,i]
            if pot_dep[j,k] > 0
                pot_test[j] = pot_nor[j,k] + pot_dep[j,k]
            end
        else
            pot_dep[j,k] = 0
            pot_nor[j,k] = 0
    ...
    return pot_test::Vector{Float64}
end

function CALCVAL(X::Matrix{Bool},T::Matrix{Float64},j::Int64,k::Int64)
    out = 0
