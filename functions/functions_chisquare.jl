    ...
    for i in axes(X,1)
        if allocated[i] == 0
            if check_coappearance[i] <= 0
                check_cap_left = copy(cap_left)
                for k in eachindex(check_cap_left)
                    if check_cap_left[k] < sku_weight[i]
                        check_cap_left = maximum(check_cap_left)
                    end
                end
                k = argmin(check_cap_left)
...
function BESTSELLING_SKUS(
    ...
    (sum(capacity_left)-sum(sku_weight))/(size(capacity_left,1)-1)
end
...
function BESTSELLING_ALLOCATE!(
    ...
    normal_weight = zeros(Float64,length(sku_weights))
...
end    # function used to perform the chi square test of independence upon 
# the coappearance matrix Q
function HYOPTHESISTEST!(dep::Matrix{<:Real},
                        Q::Matrix{<:Real},
                        I::Int64,
